    ...
    public class SinaApi : BaseApi
    {
        ...
        public bool GetAccessToken(string code)
        {
            try
            {
                ...
                parameters.Add("client_id", this.config.GetAppKey());
                parameters.Add("client_secret", this.config.GetAppSecret());
                parameters.Add("grant_type", "authorization_code");
                parameters.Add("redirect_uri", this.config.GetCallbackUrl());
        ...
        public SinaWeibo AddWeibo(string content, float jing = 0.0f, float wei = 0.0f, string annotations = "")
        {
            try
            {
                ...
                Dictionary<object, object> parameters = new Dictionary<object, object>();
                parameters.Add("access_token", this.Token.AccessToken); //采用OAuth授权方式为必填参数，其他授权方式不需要此参数，OAuth授权后获得。
                parameters.Add("status", content);
                parameters.Add("lat", jing);
                parameters.Add("long", wei);
