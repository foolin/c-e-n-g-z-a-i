...
namespace CengZai.DAL
{
	...
	public partial class Dynamic
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "T_Dynamic"); 
		}
		...
		/// 得到一个对象实体
		/// </summary>
		public CengZai.Model.Dynamic GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Content,UserID,PostTime from T_Dynamic ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,Content,UserID,PostTime ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,Content,UserID,PostTime ");
			strSql.Append(" FROM T_Dynamic ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
